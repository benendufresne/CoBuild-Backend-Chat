...
export const _verifyUserTokenBySocket = async (params) => {
	try {
		const payload = await axiosService.getData({"url":process.env.AUTH_APP_URL+SERVER.VERIFY_AUTH_TOKEN, "body":{}, auth: 'Bearer '+params.accessToken });
		return { isValid: true, credentials: { "accessToken": payload.credentials.accessToken, "tokenData": payload.credentials.tokenData } };
	} catch (e) {
		console.log("e.response.data.error === >",e);
		// return MESSAGES.ERROR.BAD_TOKEN
		if(e.type == "SESSION_EXPIRED"){
			return Promise.reject( MESSAGES.ERROR.SESSION_EXPIRED);
		}
		if(e.type == "BAD_TOKEN"){
			return Promise.reject(MESSAGES.ERROR.BAD_TOKEN);
		}
	}	
}
