...
import { GENERATOR, MESSAGE_TYPE, MIME_TYPE, NOTIFICATION_MESSAGE_TYPE, REDIS_CLUSTER_CONFIG, REGEX, SERVER } from "@config/index";
...
const createWebPushPayload = function (data) {
  ...
  data.icon = data.icon
    ? data.icon
    : "https://s3.amazonaws.com/appinventiv-development/ustandby/15644684745409Ri3K.png";
  data.priority = data.priority ? data.priority : "high";
...
const sendNotification= async (body:ChatRequest.CHAT_NOTIFICATION, accessToken:string)=> {
	try {
		consolelog(`${body.details.receiverId}---sendChatNotification invoked chatId--->`,body.details.chatId,true)
		let receiverIds = [];
		receiverIds.push(body.details.receiverId);
		// let contact = await baseDao.findOne("contacts", { userId: body.details.receiverId , contactUserId: body.details.senderId}, { name: 1 });
		// body.title = contact?.name || body.details.receiverIdName
		const response = await axios.post(`${SERVER.NOTIFICATION_APP_URL}notification/send`,
			JSON.stringify({
		...
		consolelog(`sendNotification response`,response.data,true);	
...
const messageTypeInChat = (messageType: string) => {
	try {
		...
		// } else if (messageType == MESSAGE_TYPE.CONTACT) {
		// 	return NOTIFICATION_MESSAGE_TYPE.CONTACT;
		} else  {
...
const checkClusterNode= function (key){
	const node=crc16xmodem(key)%REDIS_CLUSTER_CONFIG.SLOTS;
	console.log('*****************checkClusterNode hash slots*************************',node);
	if(node<=REDIS_CLUSTER_CONFIG.RANGE) {
		return REDIS_CLUSTER_CONFIG.NODE_1
	}
	if(node>REDIS_CLUSTER_CONFIG.RANGE) {
		return REDIS_CLUSTER_CONFIG.NODE_2
	}
};

const OneToOneNotifications=async(params,tokenData)=>{
	try{
		const apiUrl = SERVER.NOTIFICATION_APP_URL+'notification/send';
